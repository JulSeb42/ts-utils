...
Converts a date in the format `yyyy-mm-dd` (string) to `dd fullMonth yyyy`
...
Converts a date in the format `yyyy-mm-dd` (string) to `dd shortMonth yyyy`
...
Converts a YouTube url (string) to an embed link for iframes
...
Converts an amount (number) to a formatted price
